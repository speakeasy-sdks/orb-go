...
type InvoiceLineItemSubLineItemsGrouping struct {
...
type InvoiceLineItemSubLineItemsMatrixConfig struct {
...
type InvoiceLineItemSubLineItemsTierConfig struct {
...
type InvoiceLineItemSubLineItemsType string

const (
	InvoiceLineItemSubLineItemsTypeMatrix InvoiceLineItemSubLineItemsType = "matrix"
	InvoiceLineItemSubLineItemsTypeTier   InvoiceLineItemSubLineItemsType = "tier"
)

func (e InvoiceLineItemSubLineItemsType) ToPointer() *InvoiceLineItemSubLineItemsType {
	return &e
}

func (e *InvoiceLineItemSubLineItemsType) UnmarshalJSON(data []byte) error {
	var v string
	...
	case "matrix":
		fallthrough
	case "tier":
		*e = InvoiceLineItemSubLineItemsType(v)
		return nil
	default:
		return fmt.Errorf("invalid value for InvoiceLineItemSubLineItemsType: %v", v)
	}
}

type InvoiceLineItemSubLineItems struct {
	...
	Grouping InvoiceLineItemSubLineItemsGrouping `json:"grouping"`
	// Only available if `type` is `matrix`. Contains the values of the matrix that this `sub_line_item` represents.
	MatrixConfig *InvoiceLineItemSubLineItemsMatrixConfig `json:"matrix_config,omitempty"`
	Name         string                                   `json:"name"`
	Quantity     float64                                  `json:"quantity"`
	// Only available if `type` is `tier`. Contains the range of units in this tier and the unit amount.
	TierConfig *InvoiceLineItemSubLineItemsTierConfig `json:"tier_config,omitempty"`
	// An identifier for a sub line item that is specific to a pricing model.
	Type InvoiceLineItemSubLineItemsType `json:"type"`
}

type InvoiceLineItemTaxAmounts struct {
...
type InvoiceLineItem struct {
	...
	SubLineItems []InvoiceLineItemSubLineItems `json:"sub_line_items"`
	// The line amount before any line item-specific discounts or minimums.
	Subtotal string `json:"subtotal"`
	// An array of tax rates and their incurred tax amounts. Empty if no tax integration is configured.
	TaxAmounts []InvoiceLineItemTaxAmounts `json:"tax_amounts"`
}
